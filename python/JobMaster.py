...
from TaskManager import TaskFromJupyter
                ...
                if recv_dict.has_key(MPI_Wrapper.Tags.TASK_ADD):
                    ...
                    if self.master.task_scheduler.has_more_work():
                        ...
                        if not task_list:
                            ...
                            self.master.task_scheduler.is_all_task_scheduled()

	            else:
			self.master.command_q.put({MPI_Wrapper.Tags.TASK_EMPTY: '', 'uuid': current_uuid}) 
...
class JobMaster(IJobMaster):
	...
	self.read_table={}
        ...
        self.taskmgr = TaskFromJupyter()
        #master_log.debug('[Master] Appmgr has instanced')

        # load app
        self.task_scheduler = IScheduler.SimpleTaskScheduler(self, self.taskmgr, self.worker_registry)
    ...
    def register_worker(self, w_uuid, capacity=1):
	    ...
	    #'wmp': worker_module_path
            send_dict = {'wid': worker.wid, 'appid': self.task_scheduler.appid,
                         'init': self.task_scheduler.setup_worker(), 'uuid': w_uuid}
	...
	self.task_scheduler.start()
		...
		#Jupyter control pause, continue
		if self.task_scheduler.have_ctl_task():
		    task_ctl = self.task_scheduler.get_ctl_task()
		    send_dict = {MPI_Wrapper.Tags.TASK_CONTROL: task_ctl}
		    tag = MPI_Wrapper.Tags.TASK_CONTROL
			...
			self.server.send_string(send_final, len(send_final), str(uuid), tag)	
                ...
                if not self.task_scheduler.has_more_work() and not self.task_scheduler.has_scheduled_work():
                    ...
                    if not self.worker_registry.checkRunning():
                        ...
                        #check all worker finalized
                        if  self.worker_registry.size() == 0:
